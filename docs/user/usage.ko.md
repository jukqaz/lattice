...
- **AI/agent:** 설치는 ephemeral로 유지합니다. 사용자 profile, `~/.cargo/bin`, shell
  config, rustup 상태, package-manager state를 변경하지 않습니다. 이미 선택된 Cargo
  toolchain으로 작업 단위 temporary root에 설치하고 해당 binary를 직접 호출합니다.
...
실제 HOME에서 restore부터 실행하지 마세요. 첫 도입은 읽기 전용 점검에서 시작해
검토된 backup 하나를 만든 뒤, 그 backup이 안전하다고 확인된 후에만 restore
test로 넘어갑니다.

1. Discovery로 시작합니다. 이 명령은 Lattice config를 변경하지 않습니다.
...
2. 낮은 위험의 service 하나를 고릅니다. 흔한 layout과 맞으면 app catalog를
   ...
   맞는 app entry가 없으면 명시적인 service config를 사용합니다.
...
3. 어떤 쓰기 작업보다 먼저 validate와 preview를 실행합니다.
   ...
   Plan에 secret, token, session file, database, cache, generated state, 또는 모르는
   path가 포함되면 멈춥니다. Include/exclude를 좁히고 dry run을 다시 실행하세요.

4. 첫 쓰기 작업은 restore가 아니라 backup이어야 합니다.
...
Snapshot history를 확인하고 destructive undo 전에는 dry-run으로 rollback을 검토합니다.
...
## 8. 보수적인 service 후보 찾기

Discovery는 service config를 쓰지 않고 local service/app 후보를 제안합니다.
기본적으로 secret, auth, session, cache, database, large-file로 보이는 path는
제외합니다.
...
제안을 검토한 뒤 `lattice app add` 또는 `lattice service add`로 명시적으로 추가합니다.
...
`--only`와 `--exclude`로 status, backup, diff, restore, 읽기 전용 group
status/plan 작업을 특정 tracked path로 좁힐 수 있습니다. Shell이 glob selector를
expand하지 않도록 quote합니다.
...
Selector는 group command에서 사용해도 path-scoped입니다. `group status`와
`group plan`은 같은 path selector를 각 member service에 적용할 뿐, batch backup
또는 restore orchestration을 추가하지 않습니다.

## 12. Service Groups

여러 기존 service가 하나의 검토 가능한 묶음이라면 group을 사용합니다. Group은
service별 TOML이 아니라 global config file에 둡니다.
